...
function createComment(request) {
  ...
  if (requestComment && requestComment.body &&
      requestComment.username && database.users[requestComment.username] && requestArticle) {
    const comment = {
    ...
    database.comments[this.comment.id] = this.comment;
    database.article.comments[this.comment.id]=comment;
    database.users[this.comment.username].commentIds.push(this.comment.id)
    database.article[this.comment.articleId].comments.push(this.comment.id);
...
function updateComment(url, request) {
  ...
  if (!id || !requestArticle || !requestComment) {
    response.status = 400;
  } else if (!savedComment || !savedArticle) {
    response.status = 404;
  } else {
    savedArticle.title = requestArticle.title || savedArticle.title;
    savedArticle.url = requestArticle.url || savedArticle.url;
...
function deleteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const savedComment = database.comments[id];
  const requestComment = request.body && request.comment.body;
  const response = {};

  if (savedComment && requestComment && savedComment.id) {
...
function upvoteComment(url, request) {
  ...
  if (savedArticle && database.users[username] && requestComment) {
    savedArticle = upvote(savedArticle, username);
